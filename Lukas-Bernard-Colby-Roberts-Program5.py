...
# ---------------------------------------
# Test your mental might with this fun little game from Cracker Barrel!
# ---------------------------------------
    ...
    def game_over(self):
        ...
        for columns in range(self.board.shape[1]):
            for rows in range(self.board.shape[0]):
                if (self.board[rows][columns] == False):
                    #above space open
                    row = rows - 1
                    if (row in range(self.board.shape[0])):
                        if (self.board[row][columns] == True):
                            row -= 1
                            if (row in range(self.board.shape[0])):
                                if (self.board[row][columns] == True):
                                    answer = False
                    #above space to the right
                    row = rows - 1
                    column = columns + 1
                    if(row in range(self.board.shape[0])) and (column in range(self.board.shape[1])):
                        if (self.board[row][column] == True):
                            ...
                            if(row in range(self.board.shape[0])) and (column in range(self.board.shape[1])):
                                if (self.board[row][column] == True):
                                    answer = False
                    #to the right of open space
                    column = columns + 1
                    if (column in range(self.board.shape[1])):
                        if (self.board[rows][column] == True):
                            column += 1
                            if (column in range(self.board.shape[1])):
                                if (self.board[rows][column] == True):
                                    answer = False
                    #Below and right of open space
                    row = rows + 1
                    column = columns + 1
                    if(row in range(self.board.shape[0])) and (column in range(self.board.shape[1])):
                        if (self.board[row][column] == True):
        ...
        if ((self.board[rstart][cstart] == True) and (self.board[rend][cend] == False)):
            if((rend == rstart) and (cend == cstart+2)):
                if ((self.board[rstart][cstart+1] == True) and (self.board[rstart][cstart+2] == False)):
                    a = True
            elif((rend == rstart) and (cend == cstart-2)):
                if ((self.board[rstart][cstart-1] == True) and (self.board[rstart][cstart-2] == False)):
                    a = True
            elif((rend == rstart-2) and (cend == cstart)):
                if ((self.board[rstart-1][cstart] == True) and (self.board[rstart-2][cstart] == False)):
                    a = True
            elif((rend == rstart+2) and (cend == cstart)):
                if ((self.board[rstart+1][cstart] == True) and (self.board[rstart+2][cstart] == False)):
                    a = True
            elif((rend == rstart+2) and (cend == cstart-2)):
                if ((self.board[rstart+1][cstart-1] == True) and (self.board[rstart+2][cstart-2] == False)):
                    a = True
            elif((rend == rstart+2) and (cend == cstart+2)):
                if ((self.board[rstart+1][cstart+1] == True) and (self.board[rstart+2][cstart+2] == False)):
                    a = True
                ...
                if ((self.board[rstart-1][cstart+1] == True) and (self.board[rstart-2][cstart+2] == False)):
                    a = True
            elif((rend == rstart-2) and (cend == cend-2)):
                if ((self.board[rstart-1][cstart-1] == True) and (self.board[rstart-2][cstart-2] == False)):
                    a = True

        return a
    
    def make_move(self, rstart, cstart, rend, cend):
